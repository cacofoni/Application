...
(function() {
  ...
  $(document).on('keypress', '[behavior~=room_speaker]', function(event) {
    if (event.keyCode === 13) {
      if (!isTextboxDisabled(event)) {
        ...
        event.target.value = '';
        event.preventDefault();
        return $(".comment").eq(0).addClass("green");
      } else {
        if ($(".alert").text() === "") {
  ...
  getPhrase = function() {
    var phrase, phrases;
    phrases = ["Imma let you finish but..", "Please wait your turn", "Better luck next time", "You could try typing faster next time", "Advice: Try pressing enter faster", "Someone beat you to it", "My one year old types faster than you", "Get ready to speak"];

...
(function() {
  var getPhrase, isTextboxDisabled, resetTextBox, setFirstTextBoxGreen;
  ...
  App.room = App.cable.subscriptions.create("RoomChannel", {
    ...
    received: function(data) {
      ...
      return setTimeout(resetTextBox, 3333);
    },
  ...
  $(document).on('keypress', '[behavior~=room_speaker]', function(event) {
    if (event.keyCode === 13) {
      if (!isTextboxDisabled(event)) {
        App.room.speak(event.target.value);
        event.target.value = '';
        event.preventDefault();
        $(".comment").eq(0).addClass("green");
        return $(".notice").text("Your comment was successfuly submitted");
      } else {
        if ($(".alert").text() === "") {
          return $(".alert").text(getPhrase);
        }
      }
    }
  });
  ...
  isTextboxDisabled = function(event) {
    return event.target.value.length < 1 || event.target.value.length > 189 || $("#comments").hasClass("blockEntries");
  };

...
(function() {
  var addMessageCountForDistractedUser, isTextboxDisabled, resetTextBox, setFirstTextBoxGreen, setNotice;
  ...
  App.room = App.cable.subscriptions.create("RoomChannel", {
    ...
    received: function(data) {
      ...
      return addMessageCountForDistractedUser();
    },
  ...
  $(document).on('keypress', '[behavior~=room_speaker]', function(event) {
    var textContent;
    if (event.keyCode === 13) {
      textContent = event.target.value.trim();
      if (!isTextboxDisabled(textContent)) {
        App.room.speak(textContent);
        event.target.value = '';
        event.preventDefault();
        $(".comment").eq(0).addClass("green");
        return setNotice("success");
      } else {
        if (textContent.length > 1) {
          $("textarea").effect("shake", {
            times: 6
          }, 500);
          return setNotice("failure");
        }
      }
    }
  });
  ...
  resetTextBox = function() {
    ...
    $("#notice").text("");
    return $("#comments").removeClass("blockEntries");
  };

  setNotice = function(indicator) {
    var phrase, phrases;
    phrases = [];
    if (indicator === "success") {
      phrases = ["You win!", "Success!", "Woooooohoooooo!", "Your comment was successfully submitted!", "Winner winner chicken dinner..", "They wish they could be like you", "Easy peasy lemon squeezy", "Nicely done!", "You could do this in your sleep", "Congrats, you win!"];
    } else if (indicator === "failure") {
      phrases = ["Imma let you finish but..", "Please wait your turn", "Better luck next time", "You could try typing faster next time", "Advice: Try pressing enter faster", "Someone beat you to it", "My one year old types faster than you", "Get ready to speak", "The red box got you", "Don't you see the red box? Wait your turn!"];
    }
    phrase = phrases[Math.floor(Math.random() * phrases.length)];
    $("#notice").removeClass().addClass(indicator);
    return $("#notice").text(phrase);
  };

  setFirstTextBoxGreen = function() {
    if ($(".comment").eq(1).hasClass("green")) {
      $(".comment").eq(1).removeClass("green");
      return $(".comment").eq(0).css('background-color', '#90EE90');
    }
  };

  isTextboxDisabled = function(textContent) {
    return textContent.length < 1 || textContent.length > 189 || $("#comments").hasClass("blockEntries");
  };

  addMessageCountForDistractedUser = function() {
    return $(window).on("blur focus", function(event) {
      var newNumber, prevType, theNum;
      prevType = $(this).data("prevType");
      if (prevType !== event.type) {
        if (event.type === "blur") {
          theNum = document.title.match(/\d+/);
          if (theNum === null) {
            return document.title = "Cacofoni: A New Messesage Awaits";
          } else {
            newNumber = +theNum + 1;
            return document.title = "Cacofoni: " + newNumber.toString() + " New Messesages Await";
          }
        } else if (event.type === "focus") {
          return document.title = "Cacofoni";
        }
      }
    });
  };
...
(function() {
  (function($) {
    var hideFeedback, hideOverlay, initFeedback, initOverlay, loading, setBoxPosition, settings, showFeedback, showOverlay, submitFeedback;
    settings = void 0;
    $.fn.feedback = function(callerSettings) {
      ...
      return $(settings.tabControls).click(function() {
        loading();
        $(settings.modalContent).load(settings.formUrl, null, function() {
          return $(settings.form).submit(submitFeedback);
        });
        return false;
      });
    };
    submitFeedback = function() {
      ...
      $.ajax({
        ...
        success: function(msg, status) {
          ...
          return $(settings.modalWindow).fadeOut(2000, function() {
            return hideFeedback();
          });
        },
    ...
    showOverlay = function() {
      return initOverlay().show();
    };
    hideOverlay = function() {
      if ($(settings.overlay).length === 0) {
        return false;
      }
      return $(settings.overlay).remove();
    };
    ...
    setBoxPosition = function() {
      ...
      return $(settings.modalWindow).css({
        top: scrollTop + (clientHeight / 10) + "px"
      });
    };
    return loading = function(text) {
      showOverlay();
      initFeedback();
      if (text == null) {
...
(function() {
  var getRandomNews, showRandomNews;

  $(document).ready(function() {
    getRandomNews();
    $('#feed').css('opacity', 0).slideDown(5000).animate({
      opacity: 1
    }, {
      queue: false,
      duration: 10000
    ...
    showRandomNews();
    return $('#comment').simplyCountable();
  });

  showRandomNews = function() {
  ...
  getRandomNews = function() {
    ...
    return $('.marquee a').eq(member).show("slideDown");
  };
